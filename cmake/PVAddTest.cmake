  ...
  if (NOT ERROR_STATE)
    ...
    if (PARAMS)
      ...
      foreach (PARAM ${PARAMS})
        ...
        if (NOT PARSED_ARGS_MPI_ONLY)
          ...
          add_test(NAME ${TEST_NAME} WORKING_DIRECTORY ${CMAKE_CURRENT_BINARY_DIR}
            COMMAND ${MPIEXEC} -n 1 ${PV_SYSTEM_TEST_COMMAND} ${TEST_BINARY} ${TEST_FLAGS} -p ${TEST_PARAMS} -l ${TEST_LOG})
          set(FIRST_TEST OFF)
        ...
        if (PV_USE_MPI AND MPI_FOUND AND NOT PARSED_ARGS_NO_MPI)
          ...
          while((COPIES EQUAL MIN_MPI_COPIES OR COPIES GREATER MIN_MPI_COPIES) AND
            ...
            set(TEST_PARAMS "input/${PARAM}.params")
            add_test(NAME ${TEST_NAME} WORKING_DIRECTORY ${CMAKE_CURRENT_BINARY_DIR}
              COMMAND ${MPIEXEC} -n ${COPIES} ${PV_SYSTEM_TEST_COMMAND} ${TEST_BINARY} ${TEST_FLAGS} -p ${TEST_PARAMS} -l ${TEST_LOG})
      ...
      if (NOT PARSED_ARGS_MPI_ONLY)
        ...
        set(TEST_LOG "${TEST_LOG_DIR}/${TEST_NAME}.log")
        add_test(NAME ${TEST_NAME} WORKING_DIRECTORY ${CMAKE_CURRENT_BINARY_DIR}
          COMMAND ${MPIEXEC} -n 1 ${PV_SYSTEM_TEST_COMMAND} ${TEST_BINARY} ${TEST_FLAGS} -l ${TEST_LOG})
        set(FIRST_TEST OFF)
        set(PREV_TEST_NAME ${TEST_NAME})
      endif()
      ...
      if (PV_USE_MPI AND MPI_FOUND AND NOT PARSED_ARGS_NO_MPI)
        ...
        while((COPIES EQUAL MIN_MPI_COPIES OR COPIES GREATER MIN_MPI_COPIES) AND
          ...
          add_test(NAME ${TEST_NAME} WORKING_DIRECTORY ${CMAKE_CURRENT_BINARY_DIR}
            COMMAND ${MPIEXEC} -n ${COPIES} ${PV_SYSTEM_TEST_COMMAND} ${TEST_BINARY} ${TEST_FLAGS} -l ${TEST_LOG})
